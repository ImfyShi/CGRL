...
typedef double        iptype;  /* item profits */
typedef long          stype;   /* sum of weight   */
typedef double        sptype;  /* sum of pofit */
...
typedef struct { /* all problem information */
    ...
    sptype    z;               /* current solution        */
    sptype    zstar;           /* optimal solution        */
    stype    zwsum;           /* weight sum of zstar     */
    itype    ws, wt;  /* items for deriving bounds */
    iptype   ps, pt;  /* items for deriving bounds */
    ...
    sptype    psumb;           /* profit sum up to b      */
...
void initfirst(allinfo *a, sptype ps, stype ws)
{
    register state *k;
...
void copyproblem(item *f, item *l, double *p, int *w, int *x)
{
    register item *i, *m;
    register int *ww, *xx;
    register double *pp;
...
sptype minknap(int n, double *p, int *w, int *x, int c)
{
    allinfo a;
    item *tab;
    interval *inttab;
...
double n5(){
    ...
    printf("c is %d \n", c);
    for(int idx=0; idx < n; idx++){
        printf("p[%d]=%f, ", idx, p[idx]);
    }
    printf("\n");
    for(int idx=0; idx < n; idx++){
        printf("w[%d]=%d, ", idx, w[idx]);
    }
    */

    int n = 5;
    double p[5] = {0.3333, 0.3333, 0.3333, 0.3333, 0.3333};
    int w[5] = {23473, 22581, 22253, 21749, 21656};
    int x[5];
    int c = 100000;
...
double tmain(){
    ...
    int n = 22;
    double p[22] = {0.0,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625
            ,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625,0.0625,0.0,0.0625,0.
            ,0.0,0.0625};
    int w[22] = {71,70,70,70,58,57,57,55,55,53,53,53,51,51,51,49,48,27,20,18,18,16};
    int x[22];
    int c = 1000;
    /*
    for(int idx=0; idx < 200; idx++){
      w[idx] = rand()%100000;
      p[idx] = 0+1.0*(rand()%RAND_MAX)/RAND_MAX *(1-0);
    }
    */
    /*
    */
    double obj_value = minknap(n,p,w,x,c);
    printf("obj value is %f \n", obj_value);
    for(int idx = 0; idx < n; idx++){
        printf("x[%d]=%d, ", idx, x[idx]);
    }
