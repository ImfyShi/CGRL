...
class MCTS():
    ...
    def __init__(self, game, nnet, args, percentile=0.35):
        ...
        # we store action mask here
        self.Vs = {}        # stores game.getValidMoves.point for board s
        # we store action id list here
    ...
    def getActionProb(self, state, temp=1):
        ...
        """
        mctsstate=copy.deepcopy(state)
        for i in range(self.args.numMCTSSims):
            #print("Simulation:", i)
            self.search(mctsstate)

        s = self.game.stringRepresentation(state)
        counts = [self.Nsa[(s,a)]+(-np.inf if 0==state.mask[a] else 0) if (s,a) in self.Nsa else (-np.inf if 0==state.mask[a] else 0) for a in range(self.game.getActionSize())]
        #mctslegalmoves=self.game.getValidMoves(state)
        #flag=False
    ...
    def search(self, state):
        ...
        s = self.game.stringRepresentation(state)
        if s not in self.Es:
            self.Es[s] = float(self.game.getGameEnded(state))
        ...
        if s not in self.Ps:
            # leaf node
            self.Ps[s], v = self.nnet.predict(self.game.getCanonicalForm(state))
            # legalmoves = self.game.getValidMoves(state)
            valids = state.mask
            validmoves=range(self.game.getActionSize())
